...
Goal of the code – Scan the target web application for a LFI vulnerability
Logic Utilized – Firstly using the if statement we filter out invalid target application inputs. Once we find a valid target we store it in selected_target. A variable containing the payload is initialized and 
To Be Noted – N/L
